...
- if the string starts with a single quote (`’`) character, it should with a single quote character
- if the string starts with a double quote (`”`) character, it should with a double quote character
...
This logic makes this example passes our tests:
...
We now have the entire string implementation in the lexer phase. But before moving to the parser, there's a small thing we need to do in the lexer to be used in the future in the emit phase. We should have a value called `firstChar`.
...
That way, we know if the string is single quote based or double quote. This will be used in the “public” function called `isSingleQuote` returned from the lexer.

```tsx
isSingleQuote: () => firstChar === "'";
```

That will be used in the parser to create the right AST node and is crucial for the emit phase. We'll look into that later on.
...
The emit phase is a new concept in this series. This is basically the act of outputting JavaScript from a TypeScript code, depending on your configuration.
...
- [Implementing String literals](https://github.com/imteekay/mini-typescript/pull/4)
- [Handling unterminated strings](https://github.com/imteekay/mini-typescript/pull/12)
- [Fix string literal test](https://github.com/imteekay/mini-typescript/pull/14)
