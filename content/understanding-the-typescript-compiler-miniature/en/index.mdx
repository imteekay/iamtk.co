...
It will keep scanning forward until it reaches the end of the number or the end of the source code. That's it. Before any operation starts in the lexer, we store the `start` position. When it finishes scanning the entire number, the position `pos` went to the last index of the number. To get the `text`, we should just slice that:
