...
This is the 6th post of the **[TypeScript Compiler Series](/series/the-typescript-compiler)** and these are the posts we talked about before:
...
Or even typed variable declarations (with different types):
...
And also remove the transformation for `Var` nodes as `Var` is not a statement anymore.
...
We would have the same JS output:
...
In this piece of content, my goal was to show the whole implementation declaring multiple symbols for variables:
