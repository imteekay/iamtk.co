...
We first compute the derivative of the loss function with respect to the output: <InlineMath>{`{\\frac{\\partial L_t}{\\partial o_i}}`}</InlineMath>.

We want to find how the loss changes with respect to a change in a single element <InlineMath>{`{o_i}`}</InlineMath> of the output <InlineMath>{`{o}`}</InlineMath>. We can use the chain rule:
...
<BlockMath>{`\\frac{\\partial L_t}{\\partial o_t} = y_t - \\hat{y}_t`}</BlockMath>
