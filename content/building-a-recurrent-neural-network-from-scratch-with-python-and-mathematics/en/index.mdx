...
Following my [ML Learning Path](/ml-ai-learning-path), after learning and implementing [Linear Regression](/building-a-linear-regression-from-scratch-with-python-and-mathematics), [Logistic Regression](/building-a-logistic-regression-from-scratch-with-python-and-mathematics), [Neural Network](/building-a-neural-network-from-scratch-with-mathematics-and-python), and [Deep Neural Network](/building-a-deep-neural-network-from-scratch), it's time to learn about sequence problems and how Recurrent Neural Networks (RNNs) handle these kinds of challenges.
...
<img
  src="/building-a-recurrent-neural-network-from-scratch-with-python-and-mathematics/001.png"
  loading="lazy"
  class="full"
/>
...
<img
  src="/building-a-recurrent-neural-network-from-scratch-with-python-and-mathematics/003.png"
  loading="lazy"
  class="full"
/>
...
Here are all the parameters and variables used in this RNN:

- <InlineMath>`U`</InlineMath>: input weight
- <InlineMath>`W`</InlineMath>: activation weight
- <InlineMath>`b`</InlineMath>: bias for the weighted sum
- <InlineMath>`a_{t - 1}`</InlineMath>: activation from previous time step
  (hidden state)
- <InlineMath>`s_t`</InlineMath>: hidden layer weighted sum
- <InlineMath>`a_t`</InlineMath>: activation from the current time step
- <InlineMath>`V`</InlineMath>: output weight
- <InlineMath>`c`</InlineMath>: bias for output weighted sum
- <InlineMath>`o_t`</InlineMath>: prediction output
...
<img
  src="/building-a-recurrent-neural-network-from-scratch-with-python-and-mathematics/002.png"
  loading="lazy"
  class="full"
/>
...
<InlineMath>`U`</InlineMath> shape is `(32, 1)`. Input shape is `(1, 1)`. The resulting
shape is `(32, 1)`.
...
<InlineMath>`a_t`</InlineMath> should be passed the `compute_gradients` method, and
then we can compute the gradient for <InlineMath>`V`</InlineMath>:
...
<InlineMath>`tanh(s_t)`</InlineMath> in this equation is basically the activation
function. In the context of the implementation, we can get this value from the hidden
state for a given time step. And this is exactly what we do:
...
This was one of the most interesting concepts I have learned about ML and Deep Learning so far. So much complexity and things going on in a RNN model internally.

For more content on ML and my learning journey, take a look at the following resources:
...
- [ML Learning Path](/ml-ai-learning-path)
