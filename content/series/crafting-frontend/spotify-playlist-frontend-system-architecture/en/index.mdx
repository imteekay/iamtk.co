...
These are my notes from a frontend architecture interview I had in the past. Here we will talk about the challenge (a Spotify Playlist), and think together about the whole architecture: API request/payload, Components, State Management, Audio Quality Preference, and some Performance Optimizations we can do.

Let's do it!

---

The challenge is the following: we should design a music playlist, similar to Spotify. It needs to list the list of songs, play a song, go to the next one, go to the previous one, and stop the song.
...
When thinking about performance optimizations, there is [a whole new world of possibilities](https://iamtk.co/web-performance-roadmap) (with [research](https://github.com/imteekay/web-performance-research)). We can discuss loading time, rendering time, images, audio optimizations, and so on.
...
But we can also think about [code splitting](https://www.iamtk.co/web-performance-code-splitting-strategies-and-react-applications) the application to deliver the bundle the user really needs.
...
Another image perf technique to improve perceived performance is to request a low-quality image with blur, download a high-quality image in the background and [smoothly transition](https://www.iamtk.co/ux-studies-with-react-typescript-and-testing-library) from the low-quality to the high-quality image with the ease-in/ease-out transition.

[insert video: smooth image transition]
