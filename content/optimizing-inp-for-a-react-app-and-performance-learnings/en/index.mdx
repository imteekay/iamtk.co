...
- [Webpack Bundle Splitting & Browser Caching](https://www.iamtk.co/webpack-bundle-splitting-and-browser-caching)
...
- `mouseup`: Pressing a key on either a physical or onscreen keyboard.
...
This is the improvement we've got for the mobile header and children components:
...
Soon [we pretend to move our application to Next.js](https://www.iamtk.co/the-evolution-of-react-rendering-architectures-and-web-performance) so we just need to remember that the server doesn't have access to the `window` object. In this case, we need to wrap it with `useEffect` or run the component with `use client`.
