...
To compute dz2, we need to do the derivative of the loss function and z2. This is basically the chain rule: dL/dz = dL/da \* da/dz, which is calculated as a2 - y, where a2 is the predicted value and y is the real label.

For the weight in the second layer, we compute this derivative: dL/dw2 = dw2 = dL/dz2 _ dz2/dw2 = (a2 - y) _ a1.T

The computation for the bias is similar to the weight but we compute the derivative of the loss function with respect to the weight in the second layer: dL/db2 = db2 = dL/dz2 \* dz2/db2. db2 = dz2.

The derivative of the loss function with respect to dz1 is a bigger chain rule: dz1 = dL/dz2 _ dz2/da1 _ da1/dz1, producing w2.T dz2 \* g’(z1).

g’(z1) is the derivative of the loss function with respect to the activation function g. In the example of this post, we need to do the derivative of the ReLU activation function.

Because a ReLU function is z if z > 0 and 0 if z ≤ 0, the derivative of it is 1 if z > 0 and 0 if z ≤ 0. In the practice example, we'll see how this is very simple to implement in Python and Numpy.

The derivative of dw1 = dL/dz2 _ dz2/da1 _ da1/dz1 _ dz1/dw1, producing dz1 _ x.T

The derivative of db1 = dL/dz2 _ dz2/da1 _ da1/dz1 \* dz1/db1, producing dz1

That's so interestingly intuitive because if we see the chain rule, we can see the backpropagation in this computation from z2 to the first parameters w1 and b1.
