...
So, instead of computing for just one value `z`, we do it for all the training examples. We symbolize it with `Z` (capital Z). Here are all the derivatives using vectorization:
...
When computing the derivatives for the weights and biases (dW1, dW2, dB1, dB2), we need to add a `1/m` term because we are not calculating for a single loss function anymore, we compute the derivative of the cost function (the mean of the total sum of the individual losses across all examples in the dataset).

This is used to normalize and take a consistent step size in the gradient descent updates. This is why it's used only for updates on weights and biases. The other parts (`dZ2`, `dZ1`) don't need this normalization.
...
===== illustration of the neural network with 2 layers, labeling the ReLU and softmax functions ===
...
To start the neural network implementation, we first need to run the forward pass, but with initialized weights and biases, in this case, `W1`, `W2`, `B1`, and `B2`.
...
===== add math equations for the forward prop ======

We divide this into two parts, the first layer (hidden layer) and the second layer (output layer).

The first layer will handle the linear combination with the input `X` and then apply the ReLU activation function.
...
====== add relu function math equation ======
...
The second layer will receive the `A1` (the output of the first layer) as an input and apply the softmax activation function to build a probability distribution over the 10 labels (digits).
...
And we finish the forward propagation. We generate `Z1`, `A1`, `Z2`, and `A2`.
...
The backpropagation needs to calculate the gradients so we can update the weights and biases in each iteration of the algorithm. It will, then, compute `dW1`, `dW2`, `dB1`, and `dB2`.

We start from the back. To compute `dW2` and `dB2`, we need to calculate `dZ2`. As we saw earlier, this is how it's calculated:

====== add equation for dZ2 ======
...
`Y` is the one hot encoding of `Y` in this context.

`dW2` and `dB2` will follow the equations:

===== add equation for `dW2` and `dB2` ======
...
Now that we have `dZ2`, `dW2`, and `dB2`, we can compute the gradients for the rest of the neural network.

`dZ1` is calculated like this:

====== add equation for dZ1 ========
...
Here we need to compute the derivative of ReLU. As we saw before, the derivative of the ReLU function if `Z1` is greater than 0 is 1, otherwise, it will be 0.
...
Then we have the `dW1` and `dB1`:

===== add equations for dw1 and db1
...
This function will receive the features (pixels) `X`, the label `Y`, a learning rate `LR`, and the number of iterations we want to do.
...
`A2` is the result (the class probabilities) in each iteration. We store it in a variable `result` to see how well it's performing.
