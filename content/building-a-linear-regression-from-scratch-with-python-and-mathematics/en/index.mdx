...
<InlineMath>`W`</InlineMath> and <InlineMath>`B`</InlineMath> are bouncing back and
forth between positive and negative with the absolute value increasing with each
iteration.
...
{/* prettier-ignore */}
<BlockMath>`\begin{equation}\begin{pmatrix} x^{(0)}_0 & x^{(0)}_1 & \cdots & x^{(0)}_{n-1} \\ x^{(1)}_0 & x^{(1)}_1 & \cdots & x^{(1)}_{n-1} \\ \cdots \\ x^{(m-1)}_0 & x^{(m-1)}_1 & \cdots & x^{(m-1)}_{n-1} \end{pmatrix}\end{equation}`</BlockMath>
...
{/* prettier-ignore */}
<BlockMath>`\begin{equation} W = \begin{pmatrix} w_0 \\ w_1 \\ \cdots \\ w_{n-1} \end{pmatrix}\end{equation}`</BlockMath>
...
<BlockMath>
  `f_{(w, b)}(x) = w_0x_0 + w_1x_1 +... + w_{n - 1}x_{n - 1} + b`
</BlockMath>
