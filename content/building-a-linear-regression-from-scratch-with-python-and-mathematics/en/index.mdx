...
We need to sum the cost for each training example using the chosen parameters <InlineMath>{`W`}</InlineMath> and <InlineMath>{`B`}</InlineMath>, so we iterate through the `m` training examples, and for each, we need to compute the function `f` and the cost, squaring the difference between `f` and the actual target value.
...
We need to compute the derivative of the cost function with respect to <InlineMath>{`W`}</InlineMath> and then again with respect to <InlineMath>{`B`}</InlineMath>. Here's the definition:
