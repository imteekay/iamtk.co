...
For the training features X, we have m training examples and n features. <InlineMath>{`X`}</InlineMath> is a matrix with dimensions (<InlineMath>{`m`}</InlineMath>, <InlineMath>{`n`}</InlineMath>) (m rows, n columns). Here's the matrix representation:
