...
In our implementation, we'll be using the mean squared error cost function, which sums the square difference between the prediction and the real target over `m` training examples.
...
We need to sum the cost for each training example using the chosen parameters `W` and `B`, so we iterate through the `m` training examples, and for each, we need to compute the function `f` and the cost, squaring the difference between `f` and the actual target value.
...
The cost function was implemented before, and now we need to implement the algorithm that minimizes the cost so we can find the most optimized parameters `W` and `B`.
...
And here is the new part. We need to iterate a number of times (a hyperparameter we can pass to the model) until it converges. In each iteration, the model will update the parameters `W` and `B`.
...
Let's break it down. `alpha` is the learning rate. It's a hyperparameter we use to control the step size to balance speed and stability when updating the parameters. Then we have the partial derivatives.

We need to compute the derivative of the cost function with respect to `W` and then again with respect to `B`. Here's the definition:

```markdown
\*frac*{\partial J(w,b)}{\partial w} &= \*frac*{1}{m} \sum\*limits*\*{i = 0}^{m-1} (f*{w,b}(x^{(i)}) - y^{(i)})x^{(i)}\\
```

```markdown
\*frac*{\partial J(w,b)}{\partial b} &= \*frac*{1}{m} \sum\*limits*\*{i = 0}^{m-1} (f*{w,b}(x^{(i)}) - y^{(i)})\\
```

This measures how much the cost changes when `W` and `B` change by a small amount. This computation is also called gradient. Let's code that.
...
- With the end of the iterations, we have the final `W` and `B`, and then we can just return them
...
- Progress slows down, though the learning rate, alpha, remains fixed

With the optimal parameters `W` and `B`, we can predict the house prices for a new dataset.
...
Before moving on to the next part of the implementation, we need to talk about the learning rate. The larger the alpha is, the faster gradient descent will converge to a solution. But, if it is too large, gradient descent will diverge.
...
`W` and `B` are bouncing back and forth between positive and negative with the absolute value increasing with each iteration.
...
For the training features X, we have m training examples and n features. `$\mathbf{X}$` is a matrix with dimensions ($m$, $n$) (m rows, n columns). Here's the matrix representation:
...
In terms of parameters, they will be a bit different. `B` will still be a scalar value. But the `W` will be a vector of n values where each weight is there for each feature in the data set. (n features, n is 4 for this dataset).
...
```markdown
$$ f*{\mathbf{w},b}(\mathbf{x}) = w_0x_0 + w_1x_1 +... + w*{n-1}x\_{n-1} + b$$
```

It's also possible to use the vector notion:

```markdown
$$ f\_{\mathbf{w},b}(\mathbf{x}) = \mathbf{w} \cdot \mathbf{x} + b$$
```
...
For fixed parameters `w` and `b`, we loop through all the features for a single example and compute the prediction following the linear model definition. Because `b` is a scalar value, we can add it later to the prediction and return it.
...
We need to compute the gradient for both `w` and `b`. The implementation looks similar to the one implemented before for a single feature linear regression. The difference is that, instead of having only one feature, we need to loop through all features to compute the gradient for `w`.
...
The gradient for `w` is a vector because we are dealing with multiple variables, so each feature needs its own weight parameter. Bias is still a scalar value.
...
- Instead of computing the gradient for `b` for each training example, compute the value in one batch
- Instead of computing the gradient for `w` for each training example and each feature, compute it in one batch
